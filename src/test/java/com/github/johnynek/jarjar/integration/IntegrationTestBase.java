...
public abstract class IntegrationTestBase {
  ...
  protected File shadeJar(File jarFile, Map<String, String> jarjarArgs, String[] rules) throws Exception {
    ...
    HashMap<String, String> originalProperties = new HashMap<>();
    if (jarjarArgs != null) {
      for (String key : jarjarArgs.keySet()) {
        originalProperties.put(key, jarjarArgs.get(key));
        System.setProperty(key, jarjarArgs.get(key));
      }
    }
    ...
    try {
      com.github.johnynek.jarjar.Main.main(typicalArgs);
    } catch (Exception ex) {
      throw new JarJarFailedException(ex);
    }finally {
